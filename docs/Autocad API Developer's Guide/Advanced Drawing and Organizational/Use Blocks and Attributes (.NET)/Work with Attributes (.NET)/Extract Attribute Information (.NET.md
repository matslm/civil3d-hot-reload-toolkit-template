---
title: "Extract Attribute Information (.NET)"
source_url: "https://help.autodesk.com/cloudhelp/2026/ENU/OARX-DevGuide-Managed/files/GUID-BA69D85A-2AED-43C2-B5B7-73022B5F28F8.htm"
hierarchy: ["ObjectARX and Managed .NET", "ObjectARX: Managed .NET Developer's Guide", "Advanced Drawing and Organizational Techniques (.NET)", "Use Blocks and Attributes (.NET)", "Work with Attributes (.NET)", "Extract Attribute Information (.NET)"]
---

# Extract Attribute Information (.NET)

You can extract attribute information from a block reference using the
`AttributeCollection` property. The
`AttributeCollection` property returns a collection of the attribute references attached to a block. When stepping through the collection, you can use the
`IsConstant` property to see if an attributes value can be changed or not.

You do not need template files to extract attribute information, and no attribute information files are created. Simply iterate the collection of attribute references, using the
`Tag` and
`TextString` properties of the attribute reference to examine the attribute information. You will also need to check to see if
`IsMTextAttribute` is
`True` or
`False`, if True you will need to get the text value for the multiline attribute with the
...
* [Create Attribute Definitions and Attribute References (.NET)](https://help.autodesk.com/cloudhelp/2026/ENU/OARX-DevGuide-Managed/files/GUID-2107599E-9405-4D8B-A6DD-83D603B41568.htm)
